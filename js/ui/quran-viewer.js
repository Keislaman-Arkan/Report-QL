    ...
    <div class="quran-modal-card bg-white w-full max-w-4xl h-full max-h-[88dvh] sm:max-h-[88vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-100 animate-in" onclick="event.stopPropagation()">
      ...
      <div class="px-4 sm:px-5 py-3.5 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800 shrink-0 gap-2">
        <div class="flex items-center gap-2.5 min-w-0">
          <div id="qv-surah-num" class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold text-base sm:text-lg border border-emerald-500/30 shrink-0">
            ${surahNo}
          </div>
          <div class="min-w-0">
      ...
      <div class="bg-slate-50 px-3 sm:px-5 py-2.5 sm:py-3 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2.5 shrink-0">
        <div class="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <!-- Surah Switcher Dropdown inside Modal -->
          <div class="flex items-center gap-1">
            <select id="qv-select-surah" onchange="switchQuranViewerSurah(this.value)" class="px-2.5 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-bold text-slate-700 outline-none max-w-[150px] sm:max-w-[180px] shadow-xs">
              ${quranSurahList.map(s => `<option value="${s.no}" ${s.no === surahNo ? 'selected' : ''}>${s.no}. ${s.name}</option>`).join('')}
            </select>
          </div>

          <!-- Target Ayah Badge -->
          <div id="qv-target-badge" class="inline-flex items-center gap-1 px-2.5 py-1.5 ${reportType === 'hafalan' ? 'bg-purple-100 text-purple-800 border-purple-200' : 'bg-blue-100 text-blue-800 border-blue-200'} border rounded-lg text-xs font-bold shadow-sm">
...
function navigateSurah(delta) {
  ...
  const numBadge = document.getElementById('qv-surah-num');
  if (numBadge) numBadge.textContent = nextNo;

  const select = document.getElementById('qv-select-surah');
  if (select) select.value = nextNo;

  const badgeEl = document.getElementById('qv-target-badge');
  if (badgeEl) {
    badgeEl.innerHTML = `<i data-lucide="bookmark" class="w-3.5 h-3.5"></i> <span>Ayat 1 - ${QuranViewerState.toAyah}</span>`;
  }
...
/**
 * Switch Surah from the modal dropdown
 */
function switchQuranViewerSurah(surahNo) {
  surahNo = parseInt(surahNo);
  if (!surahNo) return;
  QuranViewerState.currentSurahNo = surahNo;
  QuranViewerState.fromAyah = 1;
  const meta = quranSurahList.find(s => s.no === surahNo);
  QuranViewerState.toAyah = meta ? meta.ayat : 10;
  QuranViewerState.filterOnlyTargetRange = false;
  
  const numBadge = document.getElementById('qv-surah-num');
  if (numBadge) numBadge.textContent = surahNo;
  ...
  const filterBtn = document.getElementById('qv-filter-range-btn');
  const filterLabel = document.getElementById('qv-filter-range-label');
  if (filterBtn) filterBtn.className = 'px-2.5 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 rounded-lg text-xs font-semibold flex items-center gap-1 transition';
  if (filterLabel) filterLabel.textContent = 'Fokus Target';

  stopQuranAudio();
  loadAndRenderSurah(surahNo);
  if (window.lucide) lucide.createIcons();
}
...
window.switchQuranViewerSurah = switchQuranViewerSurah;
window.openQuranViewer = openQuranViewer;
window.closeQuranViewer = closeQuranViewer;
window.openQuranViewerFromForm = openQuranViewerFromForm;
window.playQuranAudio = playQuranAudio;
window.stopQuranAudio = stopQuranAudio;
window.jumpToAyah = jumpToAyah;
window.toggleQuranRangeFilter = toggleQuranRangeFilter;
window.toggleQuranOption = toggleQuranOption;
window.changeQuranFontSize = changeQuranFontSize;
